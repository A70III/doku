...
| 33 | พื้นผิว / สี / ระยะ | **warm neutral + accent เดียว** — surface/line/text อยู่ตระกูลอุ่นเดียวกัน · ไม่ใช้ `#fff`/`#000` ล้วน · accent (default `#3b7df0` ตามข้อ 5) ใช้กับ *สถานะปัจจุบัน* เท่านั้น · semantic อิ่มต่ำ ใช้เมื่อเป็น state จริง · **radius 2/4/6px** คงที่ และ **shadow เฉพาะ overlay** (menu/modal) — depth ปกติมาจาก hairline + พื้นหลังจาง + whitespace · ค่า hex เป้าหมายอยู่ที่ [03 §1.1](03-blocks-and-design-system.md) และ **ล็อกแล้ว** (ตรวจ contrast + `/styleguide` ตอน migrate) · migrate ในรอบ UI ของ **M3** |
