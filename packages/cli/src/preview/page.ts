/**
 * หน้า preview แบบ standalone (`kairn render` → stdout/ไฟล์)
 *
...
import type { Meta, TocEntry, Warning } from "@kairn/core"
...
<body>
<div class="kairn-page">
<article class="kairn-card kairn-prose">
${docHeader(input)}
${warningsBanner(input.warnings)}
${tocBlock(input.toc)}
${input.html}
</article>
<footer class="kairn-footer">kairn · ${escapeHtml(input.docId || "(inline)")} · ${escapeHtml(meta.status)}</footer>
</div>
</body>
</html>
`
...
function docHeader(input: PreviewPageInput): string {
  const { meta } = input
  const bits: string[] = [`<span class="kairn-status">${escapeHtml(meta.status)}</span>`]
  for (const tag of meta.tags) bits.push(`<span class="kairn-tag">#${escapeHtml(tag)}</span>`)
  ...
  if (input.docId) bits.push(`<span class="kairn-doc-path">${escapeHtml(input.docId)}</span>`)

  const summary = meta.summary ? `<p class="kairn-doc-meta">${escapeHtml(meta.summary)}</p>` : ""

  return `<header class="kairn-doc-header">
<h1 class="kairn-doc-title">${escapeHtml(input.title)}</h1>
${summary}
<div class="kairn-doc-meta">${bits.join("")}</div>
</header>`
}
...
function warningsBanner(warnings: readonly Warning[]): string {
  ...
  return `<div class="kairn-warnings"><strong>render warnings (${warnings.length})</strong>${seriousHtml}${infoHtml}</div>`
}

function tocBlock(toc: readonly TocEntry[]): string {
  if (toc.length < 2) return ""
  return `<nav class="kairn-toc">
<span class="kairn-toc-title">สารบัญ</span>
<ul>${toc
    .map(
      (entry) =>
        `<li class="kairn-toc-h${entry.depth}"><a href="#${encodeURI(entry.id)}">${escapeHtml(entry.text)}</a></li>`,
    )
    .join("")}</ul>
</nav>`
}
