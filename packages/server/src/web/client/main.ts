...
;(() => {
  ...
  async function newDoc(dir) {
    ...
    if (!path) return
    try {
      await jsonRequest("POST", "/api/docs/" + encodePath(path), {
        md: "# " + basename(path) + "\n\n",
      })
  ...
  async function openHistory(id, anchor) {
    try {
      ...
      const items = [
        {
          label: "เก็บ revision ของสถานะปัจจุบัน",
          run: () =>
            api("/api/revisions/" + encodePath(id), {
              method: "POST",
      ...
      for (const entry of result.items.slice(0, 20)) {
        ...
        items.push({
          label:
            (Number.isNaN(when.getTime()) ? entry.at : when.toLocaleString("th-TH")) +
            (entry.hasMeta ? " · meta" : ""),
          run: async () => {
  ...
  const MARKDOWN_ITEMS = [
    ...
    { keyword: "quote", label: "อ้างคำพูด", detail: ">", template: "> |" },
    {
      ...
      label: "โค้ด",
      detail: "code",
      template: FENCE + "ts" + LF + "|" + LF + FENCE,
    },
    {
      keyword: "table",
      label: "ตาราง",
      detail: "| … |",
      template: "| หัวข้อ | หัวข้อ |" + LF + "| --- | --- |" + LF + "| | |",
    },
  ...
  function openBlockMenu(anchor) {
    ...
    openMenu(anchor, "block #" + block.headLine + " · " + block.kind, [
      ...
      },
      {
        label: "ซ้อน (nest)",
        hint: "Tab",
        run: () => writing.handle.runBlockOp("indent", { from: block.from, to: block.to }),
      },
      {
        label: "ลดร่น (un-nest)",
        hint: "Shift+Tab",
        run: () => writing.handle.runBlockOp("outdent", { from: block.from, to: block.to }),
      },
      { separator: true },
  ...
  function updateDropTarget() {
    ...
    const next = target
      ...
      : blocks[0]
    const rect = articleEl.getBoundingClientRect()
    const depth = Math.max(0, Math.min(5, Math.round((x - rect.left - 24) / 24)))
  ...
  function syncMarkBar(el, info) {
    ...
    for (const button of el.querySelectorAll("button")) {
      ...
      const color = button.classList.contains("doku-mark-clear")
        ? null
  ...
  function paletteCommands() {
    const commands = [
      ...
      {
        label: "ไปที่ styleguide",
        hint: "styleguide",
        run: () => (window.location.href = "/styleguide"),
      },
  ...
  document.addEventListener("dragstart", (event) => {
    ...
    const row = event.target.closest
      ? event.target.closest("[data-doc-id], [data-folder-path]")
      : null
    if (!row) return
  ...
  document.addEventListener("dragover", (event) => {
    ...
    const drop = event.target.closest
      ? event.target.closest("[data-drop-folder], [data-drop-root]")
      : null
    if (!drop) return
    event.preventDefault()
  ...
  document.addEventListener("drop", async (event) => {
    ...
    if (
      moving.kind === "folder" &&
      (targetDir === moving.path || targetDir.indexOf(moving.path + "/") === 0)
    ) {
    ...
    try {
      if (moving.kind === "doc") {
        ...
        await jsonRequest("POST", "/api/folders/" + encodePath(moving.path) + "/move", {
          to: target,
        })
      }
      toast("ย้ายแล้ว: " + target)
