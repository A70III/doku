...
export const dokuSanitizeSchema: SanitizeOptions = {
  ...
  attributes: {
    ...
    // poster ใส่โดย rehypeRewrite (หาไฟล์ข้าง src) · loop/muted/controls = ไม่รับจาก markdown แล้ว
    // (controls เปิดเสมอจาก renderer — docs/08 ข้อ 65)
    video: attrs("video", [
      ["className", K],
      "src",
      "poster",
    ...
    audio: attrs("audio", [["className", K], "src", "controls", "preload", "data*"]),
    ...
    // iframe = embed ของ provider ที่ renderer สร้างเอง (YouTube — docs/08 ข้อ 65)
    // `src` ถูกตรวจซ้ำด้วย regex ที่ rehypeRewrite (หลัง sanitize) → ที่นี่เป็นแค่ allowlist ชื่อ attribute
    iframe: attrs("iframe", [
      ["className", K],
      "src",
      "title",
